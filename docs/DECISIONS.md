...
## ADR-0016: ロープレ顧客ペルソナの導入と、質問意図を読む決定的モック応答

- **日付:** 2026-07-25（Phase 4 補強）
- **Context:** デモモードのお客様AI（`customerReply`）が、料金語（「円」「月額」等）を含むだけで質問・回答を区別せず一律に「本当に下がるんですか」を返し、ヒアリングと噛み合わなかった。「月額は？」に答えず、「2970円になります」と具体額を伝えても同じ疑問文を繰り返す。原因は (1) 質問意図を読まないキーワード一致、(2) シナリオが「今の月額・データ量・世帯・不満」など、ヒアリングで必ず聞かれる具体値を持たない（ペルソナ不足）の2点。
- **Decision:**
  - `Scenario` に任意の `persona`（`ScenarioPersona`：`monthlyFee`／`dataUsage`／`household`／`painPoint`／`switchBarrier`／`personality`）を追加し、全12シナリオに付与。
  - `customerReply` を「クロージング → 質問（トピック別にペルソナ値で回答）→ 具体金額提示（今の月額と比較して反応・同じ疑問を繰り返さない）→ 具体額なしの値下げ提案（警戒度で疑う）→ 困りごと → 挨拶 → その他」の順で分岐する形に刷新。疑問判定は「？／〜か／明示的疑問語」で、平叙文（「安くなります」）を質問と誤認しない。
  - 金額整形はロケール非依存の自作 `yen()` を使い、`toLocaleString` の環境差を避けて決定性（Vitest）を維持。
  - `persona` は Scenario 丸ごと Edge Function に渡るため AI 応答経路にも自動で届く。顧客役プロンプトに「persona はあなた自身の現状設定で一貫して答える／ドコモの事実値ではない」旨の一文を追加。
  - **重要な線引き:** persona の金額・データ量は「顧客が“今”払っている他社/現状の**練習用想定値**」であり、ドコモの料金・還元などの**事実値ではない**（事実値は products が正典・非改変）。
- **Consequences:** ヒアリングに一貫して具体的に答え、金額提示には今の支払いと比較して反応するため、会話のズレと同一文の反復が解消。`persona` は任意フィールドのため既存テスト（persona 無しの Scenario）は従来どおり素直な応答にフォールバックし後方互換。roleplay.test.ts に3ケース追加（料金・データ・具体額反応）、typecheck/lint/unit(roleplay)/build 緑。既存の growth.test.tsx の8失敗は本変更前から存在する入室ゲート系UIテストで無関係。
