...
### MNP事前伝達トークの追加検証（2026-07-09）— data-integrity

- [x] **A-D19** MNP予約番号発行前の「事前伝達トーク」追加を検証。対象: scenes.ts（scene-mnp-reservation）/ curriculum.ts（l3-5 body/keyPoint/talkScript/mistakes/practice）/ quiz.ts（q-p7m1-5）/ BASIC_DESIGN・REQUIREMENTS_SPEC（62問・16場面へ更新、実データと一致確認）。🔴ゼロ。料金・還元・解約金の具体数値の捏造なし（「解約金がかかる可能性」は伝聞・可能性の枠）。断定保証「解約金は絶対にかからない」は誤答化し explanation で否定。判断機会を奪う「全部無視/聞き流す」は NG/誤答として明示ラベル化し、正答側は断り文句提供＋保留の逃げ道＋「最終判断はお客様に残す」で本人判断・本人対応原則と整合。scenes/curriculum/quiz は3点セット（予告・出やすい話・断り方）で相互一致。
- [x] **A-D20 🟡(data/compliance)** 監査指摘を反映: 他社運用の一般化断定「引き止め窓口につながります」→「つながることが多い」に緩和、競合オペレーターの意図断定「わざとゆっくり」→「ゆっくり長くお話しされる場合も」に中立化。三社共通運用の実態は現場責任者（高橋）の口頭指示由来（公式ソース照合対象外の現場ナレッジ）。

### ロープレ顧客ペルソナ導入の事実値の位置づけ（2026-07-25）— data-integrity

- [ ] **A-D21 🟡(data)** ロープレ各シナリオに `persona`（`monthlyFee` 等）を追加（seed.ts の SCENARIOS 全12件）。**これらの金額・データ量は「顧客が“今”払っている他社/現状の練習用想定値」であり、ドコモの料金・還元・補償などの事実値ではない。** ドコモ側の数値（products / talk_scripts）は一切変更していない。想定値は相場感で設定（例: 一人利用 3,000〜9,000円、家族 12,000〜23,000円）。お客様AIはこの persona で一貫回答し、提案側の数値は断定せず公式確認を促す方針（Edge Function 顧客役プロンプトにも明記）。想定値のレンジ妥当性は現場責任者（高橋）レビューで最終確認したい（練習体験に支障は無いため保留マーク）。
