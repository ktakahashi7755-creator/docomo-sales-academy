// 場面別トークスクリプト集（16シーン）。丸暗記でなく「意図」とセットで学ぶ。
...
export const SCENES: SceneScript[] = [
  ...
  {
    id: "scene-mnp-reservation",
    scene: "MNP予約番号の発行前（ポートイン手続き）",
    objective: "引き止めトークを事前に予告し、お客様が慌てず対応できるようにする",
    script: [
      {
        role: "staff",
        text: "これから、今お使いのキャリアにお電話いただいて、乗り換えに必要な「MNP予約番号」をお取りします。その前に1つだけお伝えさせてください。",
        note: "電話の前に必ず一言。予告なしで引き止めに遭うと不安が一気に膨らむ。",
      },
      {
        role: "staff",
        text: "どのキャリアでも、この電話は解約を引き止める専門の窓口につながることが多いんです。新しいプランのご案内や「解約金がかかる可能性があります」など、少し不安になるお話が出ることがあります。",
        note: "引き止め窓口の存在を事実として予告。脅かさず、淡々と。",
      },
      {
        role: "staff",
        text: "費用まわりは先ほどご一緒に確認したとおりですので、もし迷うお話が出ても「乗り換え先が決まっているので大丈夫です」「結構です」とお伝えいただければ大丈夫です。ゆっくり長くお話しされる場合もありますが、慌てなくて大丈夫ですよ。",
        note: "断り方の言葉をそのまま渡す。費用は事前確認済みが前提（その場の新情報で判断させない）。",
      },
      { role: "customer", text: "わかりました。そう言えばいいのね。" },
      {
        role: "staff",
        text: "はい。もし気になるご案内が出たら、いったん保留にして私に聞いていただいても大丈夫です。それではおかけしますね。よろしくお願いします。",
        note: "逃げ道（保留して相談）も渡す。最終判断はお客様に残す。",
      },
    ],
    intent: "引き止めトークの内容を先に知らせ、不安と迷いを最小化する（プリフレーム）",
    salesTechnique: "事前予告（プリフレーム）と断り文句の提供",
    ngExpression: "「引き止めは全部無視してください」（判断機会を奪う強要はNG）",
    rookieExplanation:
      "MNP予約番号の発行電話は、docomo・au・ソフトバンクのどこでも引き止め専門のオペレーターにつながる。予告なしだとお客様が不安になり、乗り換え自体が崩れることがある。電話の前の一言が成否を分ける。",
    advancedTip:
      "端末の残債や違約金の有無は電話の前に一緒に確認しておく。電話中に「解約金がかかるかも」と言われても、事前確認済みならお客様が自分で落ち着いて判断できる。",
  },
];
