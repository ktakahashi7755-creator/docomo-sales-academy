...
import { CURRENT_USER } from "@/growth/data/curriculum";
import { ProgressRing } from "@/growth/components/ui";

export function Header() {
  const u = CURRENT_USER;
  return (
    <header className="sticky top-0 z-10 border-b border-slate-100 bg-white/85 px-5 py-3.5 backdrop-blur md:px-7">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="hidden items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-3 py-1.5 md:flex">
            <ProgressRing value={u.progress} size={40} stroke={4} label={`${u.progress}%`} />
            <div className="leading-tight">
              <div className="text-[11px] text-slate-500">現在の進捗</div>
              <div className="text-sm font-semibold text-slate-800">{u.stepLabel}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden leading-tight sm:block">
              <div className="text-sm font-semibold text-slate-800">{u.name} さん</div>
