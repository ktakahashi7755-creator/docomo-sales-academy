...
座学・理解度テスト・キャッチ・トークスクリプト・ロープレ・AI評価・認定までを一本の道筋に設計しています。

## キャッチ研修コンテンツ
キャッチはただのティッシュ配りではなく、携帯電話を変えるつもりのないお客様に対して
**興味付け → 着座 → 提案 → クロージング → 店舗スタッフ引継ぎ** までを設計する「入口の営業」として教材化しています。

- `/catch` … キャッチとは / 5フェーズ設計図 / 心構え / 1日の目安ファネル / NGルール
- `/scripts` … 掛け合い形式（お客様の返答・各セリフの狙い付き）のトークスクリプト
  - キャッチ基本形：ティッシュ配布から着座まで
  - 抽選会フック：イベントから自然に料金診断へ
  - 着座後：料金診断から提案・店舗スタッフ引継ぎまで
  - キャッチの断り文句：切り返しパターン集

データは `src/data/catchScripts.ts`（掛け合いは `TalkSection.dialogue`、フェーズは `phases` で管理）。
